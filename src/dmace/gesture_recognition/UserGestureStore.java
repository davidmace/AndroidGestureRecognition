...
public class UserGestureStore {
	
	static int numGestures=2,numFeatures;
	static double[][][] weights;
	static GestureFeatureAspects[][] gfa;
	static ArrayList[] tVectors;
	public static Activity context;
	
	public UserGestureStore(int numGestures) {
		tVectors=new ArrayList[numGestures];
		for(int i=0; i<numGestures; i++) tVectors[i]=new ArrayList<double[]>();
	...
	public static void train(double[] vector, int type) {
		if(vector==null) return;
		
		tVectors[type].add(vector);
		numFeatures=82;
		gfa=new GestureFeatureAspects[numGestures][numFeatures];
		weights=new double[numGestures][numGestures][numFeatures];
	...
	public static int query(double[] data) {
		...
		if(tVectors[0].size()==0 || tVectors[1].size()==0) {
			Toast.makeText(context.getApplicationContext(), "Need At Least 1 Training Sample Per Gesture Type", Toast.LENGTH_SHORT).show();
			return 0;
		}
